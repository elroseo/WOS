---
name: Tanya Sheoran
role: Customer Reliability Engineer III
team: Customer Reliability Engineering
manager: Melanie King
github: tanyasheoran
last_updated: 2026-08-13
---

# Tanya Sheoran

## Role and context

- **Position:** Customer Reliability Engineer III
- **Team:** Customer Reliability Engineering
- **How we interact:** Mentoring and CRE learning

## Strengths to learn from

- Distinguishing routine account noise from meaningful operational risk
- Proactive account monitoring using validated queries and alerts
- Finding engineering ownership from API routes and technical evidence
- Setting clear boundaries between evidence gathering and extended live troubleshooting
- Connecting technical signals to practical customer action

## Notes from recent interactions

- Mentoring session held August 13, 2026.
- Demonstrated a secondary rate-limit investigation and alert workflow.
- Encouraged moving from passive shadowing toward hands-on ticket and account participation.

## Action items

- See [[Tanya Sheoran - 2026-08-13 Mentoring]] for meeting notes.
- Ask for a follow-up walkthrough from alert detection through resolution.
