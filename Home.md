# 🏠 Home

Welcome to your Work Operating System.

## Quick Links

### Daily
- [[Task Dashboard - Work]] - current tasks
- [[Task Inbox]] - quick capture
- Today's daily file: [[Work/Daily/]]

### Prep
- [[Work/People/]] - people notes
- [[Work/Notes/Meeting Notes/]] - meeting notes
- [[Work/Notes/Hub Notes/]] - hub notes

### Learn
- [[Work/GitHub/Training & Development/Certifications/GitHub_Foundations/00_Plan/Study Plan|GitHub Foundations Study Plan]]
- [[Work/Cheatsheets/]]

### Reference
- [[Work/Voice/Writing Style|Writing Style]]
- [[Work/Voice/Goals|Goals]]

---

## Daily Routine

| When | What | Command |
|------|------|---------|
| Morning | Daily briefing | `/today` |
| Before meetings | Meeting prep | `/meeting [name]` |
| End of day | Wrap up and summary | `/eod` |
| Friday | Weekly review | `/week` |

---

> [!tip] Quick actions
> - **⌘+P** > "Open today's daily note" for journal
> - **⌘+P** > "Templates: Insert template" for meeting notes
> - **⌘+P** > "Tasks" to add a task from anywhere
